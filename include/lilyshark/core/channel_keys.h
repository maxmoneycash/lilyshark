...
// device, so decoders can read traffic on channels they are entitled to read.
// It is deliberately separate from `AppSettings`: preferences are written on
// every brightness nudge and are printed to the serial log during diagnostics,
// and key material must not ride along with either.
//
// ── Where the keys live ─────────────────────────────────────────────────────
...
//     half-applied, so the decoder never runs on a key nobody entered.
//   * IN SCOPE: a setup reset erases the keys (see below).
...
//     `RawFrame` has no key field and gains none. `DecodedPacket` — which does
//     record *which* key read a frame — is never written to either format.
//   * `DecodedPacket` carries a slot index and a flag, never bytes. Everything
//     downstream of the decoder, capture files included, can therefore only
//     ever learn that a key matched, not what it was.
...
//     key material, and its only caller is the Meshtastic payload reader.
//
// ── Setup reset ─────────────────────────────────────────────────────────────
// "RUN SETUP AGAIN" clears the store and erases the stored record. Unlike
// every other preference, a failed erase is NOT rolled back: the in-memory
// store stays cleared so the keys are unusable for the rest of the session,
// and the failure is reported as an error naming the risk that a copy remains
// in flash. Restoring secrets because a write failed would be the wrong way
// round.
//
// ── Fingerprints ────────────────────────────────────────────────────────────
...
class ChannelKeyStore final : public ChannelKeyProvider
{
  public:
    ...
    /// secret cannot be stored twice under two names and read twice in the
    ...
    /// ChannelKeyProvider. Decoders see the store through these two calls and
    /// nothing else; `channelKeyBytes` is the single egress for key material.
