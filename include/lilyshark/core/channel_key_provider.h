...
//
// Decoders take a `const ChannelKeyProvider *`, never a key store and never a
// buffer of keys, so the storage layer and the decoding layer share exactly
// one narrow interface: an ordered count and a pointer to sixteen bytes.
// Nothing here can name, list, print, or serialise a key, and a null provider
// is the normal keyless case rather than an error.
