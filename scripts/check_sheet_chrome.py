...
2. .navigationBarTitleDisplayMode is @available(macOS, unavailable) and
   ...
   PommeCore-macOS target. No CI job builds it and build_ios.sh does not,
   so the compiler is not going to be the one to tell you -- but it IS
   buildable from this checkout, and an earlier version of this docstring
   claimed it was not. `xcodebuild -scheme PommeCore-macOS -destination
   'platform=macOS' CODE_SIGNING_ALLOWED=NO build` succeeds on a fixed tree
   and fails on an unguarded one with the exact unavailability error;
   ios/BUILD.md already documented that command as working. Two such lines
   were added by hand in 8c6a2fa and sat there unnoticed.

Only sheet PRESENTATION sites are checked -- a dismiss button declared inside
a `.sheet` closure. A dismiss button declared OUTSIDE one, in a view's own
body, is not caught: there this script cannot tell a sheet from a pushed
screen, and guessing wrong would flag the contact list, whose large title is
deliberate.

That limit is real and worth stating plainly, because it means this check does
NOT protect three of the fixes it shipped alongside -- NetworkToolsView,
FirmwareUpdateView and GeofencesView all declare their dismiss button in their
own body, and reverting any of them leaves this script at exit 0. The macOS
