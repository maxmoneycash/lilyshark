/**
 * Windowed rendering math for the frame table (UI-012).
 *
 * A capture is now allowed to be large — an overnight field session is
 * hundreds of thousands of frames — so the table renders only the rows the
...
export function visibleSpan(m: {
	...
	const selfScrolls = m.scrollHeightPx - m.clientHeightPx > 1;
	if (selfScrolls)
		return {
 ...
 * The table's arrows used to walk DOM siblings, which only works while every
 * row is mounted. They walk positions instead now, so navigation is the same
 * whether the next row is rendered, three screens down, or the 200,000th
