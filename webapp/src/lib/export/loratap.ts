 ...
 * reliable RTC, so Wireshark shows the capture starting near the epoch and
 * the deltas between frames are the part that means anything.
 *
...
/** How many frames a pcap of some frame list holds, and what it left behind. */
export interface LoraTapCounts {
...
export interface LoraTapPcapResult extends LoraTapCounts {
	bytes: Uint8Array;
}

/**
 * The only fields the encodability rules look at. A caller holding nothing
 * but the device's raw record fields can therefore ask what a pcap would
 * contain without decoding every frame first; LscapFrame satisfies this
 * shape as it stands.
 */
export interface LoraTapEncodable {
	synthetic: boolean;
	bandwidthHz: number;
...
/** Why a pcap cannot carry a frame, or null when it can. */
export type LoraTapExclusion = "synthetic" | "unencodable";
...
/**
 * Decide a frame's fate before any bytes are laid out. Both the writer and
 * the counter go through this one predicate, so the number a download button
 * promises and the number the file holds cannot drift apart.
 */
export function loraTapExclusion(
	frame: LoraTapEncodable,
): LoraTapExclusion | null {
	if (frame.synthetic) return "synthetic";
	if (
		bandwidthCode(frame.bandwidthHz) === null ||
...
/**
 * What buildLoraTapPcap would write, without writing it. A live sniffer
 * relabels its download button on every frame that arrives, and assembling
 * the whole file each time only to count its records would be waste.
 */
export function countLoraTapPcap(
	frames: readonly LoraTapEncodable[],
): LoraTapCounts {
	const counts: LoraTapCounts = {
		written: 0,
		excludedSynthetic: 0,
		excludedUnencodable: 0,
	};
	for (const frame of frames) {
		const exclusion = loraTapExclusion(frame);
		if (exclusion === "synthetic") counts.excludedSynthetic++;
		else if (exclusion === "unencodable") counts.excludedUnencodable++;
		else counts.written++;
	}
	return counts;
}
...
export function buildLoraTapPcap(options: ExportOptions): LoraTapPcapResult {
	...
	for (const frame of options.frames) {
		const exclusion = loraTapExclusion(frame);
		if (exclusion === "synthetic") {
			excludedSynthetic++;
			continue;
		}
		if (exclusion === "unencodable") {
			excludedUnencodable++;
			continue;
		}
		// loraTapExclusion has already rejected every bandwidth the format
		// cannot carry, so this is a real 125 kHz step count.
		const bandwidth = bandwidthCode(frame.bandwidthHz) ?? 0;
