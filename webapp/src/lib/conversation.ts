/**
 * Follow conversation (UI-008) — the analyzer's follow-stream gesture.
 *
 ...
 * `src` and `dst` fields frameFilter.ts adds), exactly the way the ANNOUNCES
 * panel writes `dest == <hash>`: the text lands in the filter box, so it
 * stays editable, composes with everything else the operator has typed, and
 * rides along in the permalink like any other filter. The rows it leaves are
 * in capture order, which is the capture clock — the table never reorders.
 *
 * ── What each protocol actually proves ───────────────────────────────────
 *
 * Addressing is read from the dissectors and NOTHING is invented beyond what
 * they prove (lib/dissect):
 *
 *   Meshtastic  the 16-byte outer header names both endpoints — `to` and
 *               `from`, 32-bit node numbers (meshtastic.ts). Both sides of a
 ...
 *               source at all (rnode.ts) — RNS carries no sender address in
 *               the header. The conversation is everything addressed to that
 ...
 *               meshcore.ts deliberately claims no field inside that region
 *               ("the firmware proves only the arithmetic"), so no address
 *               is claimed here either. MeshCore frames are excluded, and
 *               `frameAddressing` says so out loud.
 *   anything    a frame whose capture profile named no protocol decodes no
 *               addressing — the dissector never guesses a protocol the
 *               radio did not name, and neither does this.
 *
 ...
 * (conversation.test.ts), following the trafficView.ts/captureSlots.ts
 * precedent.
 */

import { meshtasticAddressHex } from "./dissect/meshtastic";
...
/**
 * One frame's addressing, as its own dissector proves it. Both fields are
...
const REASON = {
	meshcore:
		"MeshCore v1 — the dissector claims no source or destination field in a captured frame (it proves the payload arithmetic only)",
	meshcoreShort: "MeshCore names no address the dissector proves",
	unknownProfile:
		"the capture profile names no protocol, so no addressing is decoded — the dissector never guesses one the radio did not name",
...
export function addressLabel(hexAddress: string): string {
	...
	// A 16-byte Reticulum hash is unreadable in full at this size; the
	// ANNOUNCES panel names destinations by their first 8 hex characters and
	// this says the same thing the same way.
