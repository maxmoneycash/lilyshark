 ...
 *
 * Two address fields, `src` and `dst`, are the same idea across protocols:
 * the endpoints a frame's own header PROVES, written as the hex of the
 * address bytes — 8 characters for a Meshtastic node number, 32 for a
 * Reticulum destination hash (so on a Reticulum frame `dst` and `dest` name
 * the same 16 bytes). Like `dest` they take == and != only and match whole.
 * Nothing is invented for a protocol that claims no address: MeshCore
 * frames, frames whose profile named no protocol, and frames with an
 * unreadable header have no `src` and no `dst`, so they equal no address at
 * all — lib/conversation.ts owns that arithmetic and the words for it, and
 * the follow-conversation gesture is nothing but the ordinary expression
 * these fields make.
 *
 ...
 * RNS header, and `src` and `dst` read whatever addressing the frame's own
 * protocol proves — lib/conversation owns both readers, and its tests pin
 * them against an independent reference. Both reads are gated on the frame's
