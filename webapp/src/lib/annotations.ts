...
/**
 * The same target, derived from a DECODED capture frame rather than the live
 * one -- the shape the export path holds.
 *
 * The export writes `sessionFrames(records)`, which are LscapFrames rebuilt
 * from the ring, not the HeardFrames on screen. Checking a note's witness
 * against the on-screen frame and then writing it beside a decoded one would
 * be checking a different object from the one being exported. This derives
 * the witness from the bytes actually going into the file.
 *
 * `stamp` is milliseconds here because that is what the live frame carries
 * and what the note was written with; LscapFrame keeps microseconds.
 */
export function capturedNoteTarget(frame: {
	sequence: bigint;
	timestampUs: bigint;
	bytes: Uint8Array;
}): { address: FrameAddress; witness: FrameWitness } {
	return {
		address: { scope: SNIFFER_SCOPE, frame: Number(frame.sequence) },
		witness: {
			len: frame.bytes.length,
			stamp: Number(frame.timestampUs / 1000n),
		},
	};
}
...
/**
 * Frame → note text for the frames being exported: the shape the CSV and
 * JSON writers read.
 *
 ...
 *
 * Keyed by the FRAME OBJECT, not by its sequence number. That is the whole
 * point of the witness and it was previously thrown away right here: this
 * function checked each frame's witness and then returned a
 * `Map<number, string>` keyed by sequence, so a run-1 frame 5 and a run-2
 * frame 5 -- which coexist in the sniffer ring, because snifferSession
 * advertises that the ring survives unplugging the deck and nothing clears it
 * on relink -- collapsed to one entry. Both rows then rendered the note, and
 * both carried it into the CSV, against bytes the operator never saw. Worse,
 * last-write-wins could hand the note to the WRONG frame and mark the frame
 * that legitimately owns it as belonging to a different run.
 *
 * An object key cannot collide, so the per-frame check the function already
 * does is the check the caller actually gets.
 */
export function notesForFrames<F extends AnnotatableFrame>(
	book: NoteBook,
	scope: NoteScope,
	frames: readonly F[],
): Map<F, string> {
	...
	for (const f of frames) {
		...
		if (note !== null && noteStanding(note, f.witness) === "same") out.set(f, note.text);
	}
	return out;
}
