...
test("proto matches the firmware's builtin profile ids", () => {
	...
	// Profile 4 is MESHTASTIC BAY MF here, not an RNode example.
	assert.equal(protoOfProfile(4), "meshtastic");
	...
	assert.equal(compile("proto != rnode")(frame({ profileId: 5 })), false);
...
test("a realistic compound filter over a small capture", () => {
	const frames = [
		...
		frame({ sequence: 3, profileId: 5, snrDb: 2, crc: "valid" }),
		frame({
			sequence: 4,
			profileId: 0,
			snrDb: 9,
...
function rnodeFrame(hashHex: string, overrides: Partial<FilterFrame> = {}) {
	...
	bytes[2] = 1; // hops
	bytes.set(hash, 3);
	return frame({ profileId: 5, bytes, ...overrides });
}
...
test("dest reads no hash out of a frame that carries none", () => {
	...
	const short = frame({ profileId: 5, bytes: new Uint8Array([0x00, 0x01]) });
