 ...
 *
 * so a divergence between the browser's boxes and that script's cells fails
...
const DECODED: Record<string, [number, number, number, number]> = {
	...
	// Palo Alto, where the demo mesh lives: the coverage layer's own cell.
	"9q9hv": [37.3974609375, 37.44140625, -122.1240234375, -122.080078125],
...
test("encode matches the Python encoder", () => {
	...
	// f.geohash_encode(37.4419, -122.143, 5) -> '9q9jh' (the demo mesh's own
	// coordinate) and f.geohash_encode(37.8, -122.2, 5) -> '9q9p5' (the Bay
	// Area centre the internet layer falls back to).
	assert.equal(encodeGeohash(37.4419, -122.143, 5), "9q9jh");
	assert.equal(encodeGeohash(37.8, -122.2, 5), "9q9p5");
});
...
test("neighbours match the scorer, wrap and pole cases included", () => {
	...
	assert.deepEqual(geohashNeighbors("9q9hv"), [
		"9q9hs",
		"9q9ht",
		"9q9hu",
		"9q9hw",
...
test("a non-finite coordinate is refused rather than sent to 00000", () => {
	// A NaN fails every bisection test, so an unguarded encoder would answer
	// with the grid's south-west corner and the map would draw a cell there.
	assert.throws(() => encodeGeohash(Number.NaN, 0, 5), GeohashError);
	assert.throws(
		() => encodeGeohash(0, Number.POSITIVE_INFINITY, 5),
		GeohashError,
	);
});
