...
test("an export carries only the notes written on the frames being exported", () => {
	...
	const target = { frame: 12, witness: first };
	const notes = notesForFrames(book, SNIFFER_SCOPE, [
		target,
		{ frame: 13, witness: witness(7, 1_772_600_000_000) },
		{ frame: 99, witness: witness(4, 1) },
	]);
	// Keyed by the frame OBJECT, so two frames sharing a sequence number
	// cannot collapse into one entry -- see the next test.
	assert.deepEqual([...notes.entries()], [[target, "interferer starts"]]);
});

test("two frames sharing a sequence number do not share a note", () => {
	// The sniffer ring survives unplugging the deck, and nothing clears it on
	// relink, so run-1 frame 5 and run-2 frame 5 coexist in one session. The
	// note was written on the first. Keyed by sequence number, BOTH rows
	// carried it and both went into the CSV -- an operator's words exported
	// against bytes they never saw.
	const runOne = witness(12, 1_772_409_600_000);
	const runTwo = witness(33, 1_772_500_000_000);
	const book = write(EMPTY_NOTES, at(5), "interferer starts", runOne);

	const fromRunOne = { frame: 5, witness: runOne };
	const fromRunTwo = { frame: 5, witness: runTwo };
	const notes = notesForFrames(book, SNIFFER_SCOPE, [fromRunOne, fromRunTwo]);

	assert.equal(notes.size, 1, "exactly one frame carries the note");
	assert.equal(notes.get(fromRunOne), "interferer starts", "the frame it was written on");
	assert.equal(
		notes.get(fromRunTwo),
		undefined,
		"and not the later frame that merely reuses the number",
	);
});
