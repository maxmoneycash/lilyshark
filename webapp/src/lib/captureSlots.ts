/**
 * Capture slots: more than one capture open at a time.
 *
 ...
 * filter, parse note): switching slots swaps all of it at once, which is what
 * keeps two captures from bleeding into each other. It is also the
 * precondition for the capture diff, which needs to be handed two captures and
 * until now had nowhere to get the second one from.
 *
 ...
 * ── Memory bound ────────────────────────────────────────────────────────
 *
 * MEASURED, not asserted, by scripts/measure-capture-retention.ts on node
 * v22.22.1 (V8, the same engine class the browser runs). Parsing a capture and
 * building the index arrays the TRAFFIC view keeps retains 569 B per frame,
 * flat from 50,000 to 200,000 frames and independent of payload size —
 *
 *   the LscapFrame object (~28 fields), its two BigInts, its Uint8Array view,
 *   and one slot in each derived array (pointers, dest hashes, addressings,
 *   shown frames).
 *
 ...
 * so 335 B for a maximum-size LoRa frame and 120 B for the 40 B frames a mesh
 * channel actually carries (both figures also printed by that script).
 *
 *   570 B (measured, rounded up) + 335 B (worst-case file bytes) = 905 B
 *                                                               → 1 KiB/frame
 *
 ...
 * of maximum-size frames come to 500 MiB and four realistic ones (689 B per
 * frame at a 40 B payload) to 336 MiB. A recording stops itself at the limit
...
/** Worst-case bytes retained per frame — 569 B measured, rounded up with the
