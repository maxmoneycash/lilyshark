/**
 * Geohash base-32 for the COVERAGE screen (UI-016).
 *
 * A Field Receipts cell is geohash-5 x band x ISO week
 * (docs/protocol/season-0.md). The season scorer publishes cells by their
 * geohash-5 string only, so the browser has to turn `u4pru` back into a box
 * to draw it. This is a straight port of `geohash_decode`, `geohash_encode`
 * and `geohash_neighbors` in scripts/field_receipts_score.py — the same
 * bisection in the same order, so both implementations produce the same
 * bytes for the same cell. geohash.test.ts pins the port against the
 * published vector below and against values read out of the Python side.
 *
 * Nothing here is a projection or a rounding: a geohash box is exact in
 * degrees, and the decode returns its edges, not its centre.
 */
