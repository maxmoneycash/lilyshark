...
/**
 * Independent readers, written from the wire formats rather than from
 * conversation.ts, so a mistake in the module under test cannot also be the
 * yardstick. The branch this port came from pinned the module against its
 * full dissector suite; that suite is not in this tree, and a cross-check
 * against a second implementation is the same discipline without it.
 */
function referenceMeshtastic(
	bytes: Uint8Array,
): { src: string; dst: string } | null {
	if (bytes.length < 16) return null;
	const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
	const destination = view.getUint32(0, true);
	const source = view.getUint32(4, true);
	if (source === 0) return null;
	const pad = (v: number) => v.toString(16).padStart(8, "0");
	return { src: pad(source), dst: pad(destination) };
}

function referenceReticulum(bytes: Uint8Array): string | null {
	if (bytes.length < 2) return null;
	if ((bytes[0] & 0x01) !== 0) return null; // split continuation
	if ((bytes[1] & 0x80) !== 0) return null; // IFAC-masked header
	const headerTwo = (bytes[1] & 0x40) !== 0;
	// RNode shim + RNS header: 1 + 19 for HEADER_1, 1 + 35 for HEADER_2.
	if (bytes.length < (headerTwo ? 36 : 20)) return null;
	if (bytes[2] >= 128) return null; // hop count out of range
	const at = headerTwo ? 19 : 3;
	return Array.from(bytes.subarray(at, at + 16), (b) =>
		b.toString(16).padStart(2, "0"),
	).join("");
}

/** Fixtures, every prefix of them, and pseudorandom bytes. */
function crossCheckCases(): Uint8Array[] {
...
test("Meshtastic addressing agrees with a second reader, byte for byte", () => {
	for (const full of crossCheckCases()) {
		for (let cut = 0; cut <= full.length; cut++) {
			const bytes = full.slice(0, cut);
			const address = frameAddressing(bytes, 1);
			const expected = referenceMeshtastic(bytes);
			if (expected === null) {
			...
			assert.equal(address.src, expected.src);
			assert.equal(address.dst, expected.dst);
		}
	}
});

test("the Reticulum hash reader agrees with a second reader, byte for byte", () => {
	for (const full of crossCheckCases()) {
		for (let cut = 0; cut <= full.length; cut++) {
			const bytes = full.slice(0, cut);
			...
			assert.equal(address.src, null);
			assert.equal(address.dst, referenceReticulum(bytes));
		}
	}
});

test("Reticulum names a destination and says it names no source", () => {
	const a = frameAddressing(reticulumBytes(RNS_HASH), 5);
...
test("a Reticulum frame with no readable header names no address", () => {
	...
	const a = frameAddressing(split, 5);
...
test("a Reticulum conversation filter follows the destination hash", () => {
	...
	assert.ok(parsed.ok);
	assert.equal(
		parsed.predicate(filterFrame(reticulumBytes(RNS_HASH), 5)),
		true,
	);
	assert.equal(
		parsed.predicate(
			filterFrame(reticulumBytes("00112233445566778899aabbccddeeff"), 5),
		),
		false,
	);
...
test("a conversation expression reads back as the conversation it follows", () => {
	for (const address of [
		...
		frameAddressing(reticulumBytes(RNS_HASH), 5),
	]) {
...
/* ── the endpoints two captures do and do not share ──────────────────── */

test("endpoints collect both ends of every addressable frame, deduplicated", () => {
	const endpoints = captureEndpoints([
		frameAddressing(meshtasticBytes(0x0badc0de, 0xa1b2c3d4), 1),
		frameAddressing(meshtasticBytes(0xa1b2c3d4, 0x0badc0de), 1),
		frameAddressing(meshtasticBytes(0xffffffff, 0x00c0ffee), 1),
		frameAddressing(reticulumBytes(RNS_HASH), 5),
		// Undecodable frames contribute no endpoint rather than a zero.
		frameAddressing(new Uint8Array([0x09, 0x00, 1, 2, 3]), 2),
	]);
	...
	assert.deepEqual(captureEndpoints([]), []);
});

test("the endpoint difference is what one capture names and the other never does", () => {
	const a = captureEndpoints([
		frameAddressing(meshtasticBytes(0x0badc0de, 0xa1b2c3d4), 1),
		frameAddressing(meshtasticBytes(0xffffffff, 0x00c0ffee), 1),
	]);
	const b = captureEndpoints([
		frameAddressing(meshtasticBytes(0xffffffff, 0x00c0ffee), 1),
	]);
	assert.deepEqual(endpointsOnlyIn(a, b), ["0badc0de", "a1b2c3d4"]);
	assert.deepEqual(endpointsOnlyIn(b, a), []);
	assert.deepEqual(endpointsOnlyIn([], a), []);
});
