...
import { buildTransitionGraph, type AnnounceTransitionGraph } from '../lib/announceGraph';
import type { AnnounceDestination, AnnounceOverview } from '../lib/announceView';
...
export function AnnouncePathGraph({
  ...
  const activeDest: AnnounceDestination | null = useMemo(() => {
    ...
    if (selectedDestHex) {
      const found = destinations.find(
        (d) => d.destinationHashHex.toLowerCase() === selectedDestHex.toLowerCase(),
      );
  ...
  if (destinations.length === 0) {
    return (
      <div className="panel traffic-paths" role="region" aria-label="Reticulum announce path transitions">
        <div className="panel-title">
          <span>RETICULUM // PATH TRANSITIONS</span>
          <span className="spacer" />
          <button type="button" onClick={onClose} aria-label="Close path transitions">✕</button>
        </div>
        <div className="traffic-empty" style={{ padding: '24px 16px' }}>
          <h2>No Reticulum announces in this capture.</h2>
          <p className="dim">Path transitions track route shifts across observed announce packets.</p>
        </div>
      </div>
    );
  }

  const minHops = activeDest ? Math.min(...activeDest.paths.map((p) => p.path.hops)) : 0;
  const maxHops = activeDest ? Math.max(...activeDest.paths.map((p) => p.path.hops)) : 0;
  const hopRangeLabel = minHops === maxHops ? `${minHops} hop${minHops === 1 ? '' : 's'}` : `${minHops} – ${maxHops} hops`;
    ...
    <div className="panel traffic-paths" role="region" aria-label="Reticulum announce path transitions">
      ...
      <div className="scroll-y" style={{ padding: '12px' }}>
        ...
        <div style={{ marginBottom: '12px' }}>
          <label htmlFor="rns-dest-select" className="k" style={{ display: 'block', marginBottom: '4px' }}>
          ...
          <select
            ...
            style={{ width: '100%', background: 'var(--bg, #111)', color: 'var(--fg, #eee)', padding: '6px 8px' }}
          >
            {destinations.map((d) => (
              <option key={d.destinationHashHex} value={d.destinationHashHex}>
                {d.destinationHashHex.slice(0, 12)}… ({d.count} announce{d.count === 1 ? '' : 's'}, {d.paths.length} path{d.paths.length === 1 ? '' : 's'})
              </option>
            ))}
          </select>
        </div>
        ...
        {activeDest && graph && (
          <>
            ...
            <div className="kv" style={{ marginBottom: '16px' }}>
              ...
              <span className="v" style={{ overflowWrap: 'anywhere' }}>{activeDest.destinationHashHex}</span>
              ...
              <span className={`v ${graph.totalTransitions > 0 ? 'warn' : 'ok'}`}>
                {graph.totalTransitions} transition{graph.totalTransitions === 1 ? '' : 's'}
              </span>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div className="panel-title" style={{ padding: '4px 0', fontSize: '11px' }}>
                DIRECTED TRANSITION EDGES ({graph.edges.length})
              </div>
              {graph.edges.length === 0 ? (
                <div className="dim" style={{ padding: '8px 0', fontSize: '12px' }}>
                  No path migrations. Destination stayed on a single path across all announces.
                </div>
              ) : (
                  ...
                  {graph.edges.map((e, idx) => (
                    <div
                      ...
                      style={{
                        ...
                        borderLeft: e.hopsDelta > 0 ? '3px solid var(--warn, #f39c12)' : e.hopsDelta < 0 ? '3px solid var(--ok, #2ecc71)' : '3px solid var(--dim, #888)',
                        fontSize: '12px',
                      }}
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                        <span style={{ fontWeight: 'bold' }}>{e.fromLabel} → {e.toLabel}</span>
                        <span className="ok">{e.count}×</span>
                      </div>
                      <div className="dim" style={{ fontSize: '11px' }}>
                        Hop shift: {e.hopsDelta > 0 ? `+${e.hopsDelta}` : e.hopsDelta} {Math.abs(e.hopsDelta) === 1 ? 'hop' : 'hops'}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              {graph.transitions.length === 0 ? (
                <div className="dim" style={{ padding: '8px 0', fontSize: '12px' }}>
                  Single steady path across all {activeDest.count} announce frames.
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '6px' }}>
                  {graph.transitions.map((t, idx) => {
                    const seq = frames[t.frameIndex] ? Number(frames[t.frameIndex].sequence) : t.frameIndex;
                    return (
                      <div
                        key={idx}
                        ...
                        style={{
                          padding: '6px 8px',
                          cursor: 'pointer',
                          background: 'var(--panel-item-bg, rgba(255,255,255,0.03))',
                          fontSize: '12px',
                        }}
                        title={`Click to inspect frame ${seq} in capture`}
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                          <span style={{ color: 'var(--accent, #64b5f6)' }}>Frame {seq}</span>
                          <span className="dim">+{t.deltaS.toFixed(1)}s</span>
                        </div>
                        <div>
                          {t.fromLabel} → <span style={{ fontWeight: 'bold' }}>{t.toLabel}</span>
                        </div>
                        <div className="dim" style={{ fontSize: '11px' }}>
                          Δhops: {t.hopsDelta > 0 ? `+${t.hopsDelta}` : t.hopsDelta}
                        </div>
                      </div>
                    );
                  })}
