...
export function TrafficTab({ demoActive }: TrafficTabProps) {
	...
	const onPublish = async () => {
		...
		try {
			...
			if (recordedSlotKey.current !== "")
				setPublishedSlotKey(recordedSlotKey.current);
	...
	const fetchBlob = async () => {
		...
		try {
			const [owner, name] = n.startsWith("0x")
				...
				: [DEMO_BLOB.owner, n];
			load(
				await fetchBlobBytes(owner, name),
				name,
				"shelby",
				`shelby:${owner}/${name}`,
			);
	...
	const autoResolveRef = useRef(
		window.location.hash.toLowerCase() === "#resolve",
	);
	useEffect(() => {
	...
	const lxmf = useMemo(() => {
		...
		return (
			dissectRNode(f.bytes, { truncated: f.truncated }).root.children.find(
				(part) => part.label === "LXMF message",
			) ?? null
		);
	}, [f]);
	...
	const following = useMemo(
		() => parseConversationExpression(filterText),
		[filterText],
	);
	...
	return (
		<main className="traffic-screen" ref={screenRef}>
			...
			>
				...
				{error && (capture || !openError) && (
					<div className="panel-foot err">{error}</div>
				)}
				...
				{link.status !== "off" && (
					<div className="kv">
						...
						{link.status === "linked" && (
							<span className="v ok">
								Lilyshark {link.firmware}
								{link.node !== undefined
									? ` · !${link.node.toString(16).padStart(8, "0")}`
									: ""}{" "}
								over USB{" "}
								{link.telemetry?.sim ? (
									<span className="sim-badge">SIMULATE MODE · SYNTHETIC</span>
								) : null}{" "}
								<button onClick={() => void disconnectDeviceLink()}>
									UNLINK
								</button>
							</span>
						)}
						{link.status === "linked" && link.telemetry && (
							<>
								...
								<span className="v">
									{reportedLabel(link.telemetry.bat)} ·{" "}
									{reportedLabel(link.telemetry.gps)} ·{" "}
									{reportedLabel(link.telemetry.profile)} · capture sequence{" "}
									{link.telemetry.frames ?? "not reported"} · latest frame RSSI{" "}
									{telemetrySignal(link.telemetry, "rssi") !== undefined
										? `${telemetrySignal(link.telemetry, "rssi")!.toFixed(1)} DBM`
										: "not reported"}{" "}
									· SNR{" "}
									{telemetrySignal(link.telemetry, "snr") !== undefined
										? `${telemetrySignal(link.telemetry, "snr")!.toFixed(1)} DB`
										: "not reported"}
								</span>
							</>
						)}
					...
					<div className="traffic-empty" role="status">
						{busy ? (
							...
							<>
								...
								<p>
									The T-Deck writes .lscap to microSD. Load the bundled sample
									to inspect 24 synthetic LongFast frames, including one Shelby
									pointer.
								</p>
							</>
						)}
							...
							{(
								[
									...
									[
										...
										<>
											<span className="ok">
												<AnimatedNumber value={stats.crcValid} /> OK
											</span>
											{" · "}
									...
									[
										"BEST SNR",
										<>
											{stats.bestSnrDb === null
												? "Not reported"
												: `${stats.bestSnrDb.toFixed(1)} DB`}
										</>,
									],
									[
										"MEDIAN RSSI",
									...
									[
										"SHELBY PTRS",
										<span className={pointerCount > 0 ? "ok" : "dim"}>
											<AnimatedNumber value={pointerCount} />
										</span>,
									],
								] as [string, ReactNode][]
							).map(([k, v]) => (
						...
						<div className="panel-foot traffic-filter">
							...
							{!filter.ok ? (
								...
								<span className="ok traffic-filter-note">
									FOLLOWING {conversationLabel(following)} · {filtered.length}{" "}
                ...
                dim footnote used to leave half the pane blank. */}
							...
							<div className="traffic-empty traffic-frames-empty" role="status">
								{filtered.length === 0 && filter.ok && !filter.empty ? (
									<>
										...
										<p>
											The expression is valid — these{" "}
											{frames.length.toLocaleString()} frames simply do not
											satisfy it.
											{f
									...
									<>
										...
										<p>
											That silence is the reading —{" "}
											{filtered.length.toLocaleString()} frame(s) match the
											filter outside this range.
										</p>
										<button type="button" onClick={() => setBrush(null)}>
											WHOLE CAPTURE
										</button>
									</>
								) : (
                   ...
                   pulled to its newest row. */
								follow={
									simulatedLive && slot !== null && slot.id === demoSlotId
								}
							/>
						)}
						...
						<div className="panel-foot">
							...
							{brush && (
								<span className="ok"> · BRUSHED {brushLabel(brush)}</span>
							)}{" "}
							· {pointerCount} SHELBY POINTER(S)
							{syntheticCount > 0 && (
								<span className="warn">
									{syntheticCount} SYNTHETIC · NOT OTA
								</span>
							)}
							{truncatedCount > 0 && (
				...
				<div className="panel traffic-detail" ref={inspectorRef}>
                  ...
                  is explained rather than merely missing. */}
							...
							{selectedAddress && isAddressable(selectedAddress) ? (
								...
								<span className="v dim">
									not addressable —{" "}
									{selectedAddress?.reason ?? "no addressing decoded"}
								</span>
							)}
							{frameIsIfac(f) && (
								<>
									...
									<span className="v warn">
										IFAC MASKED · Protected by Interface Access Code (inner
										headers and destination cannot be read without interface
										access key)
									</span>
								</>
							)}
							...
							<span className="v">
								{hasField(f, RF_FIELD.spreadingFactor)
									? `SF${f.spreadingFactor}`
									: "SF not reported"}{" "}
								·{" "}
								{hasField(f, RF_FIELD.codingRate)
									? `4/${f.codingRateDenominator}`
									: "CR not reported"}
							</span>
							...
							<section aria-label="Reticulum announce">
								...
								<dl className="kv" style={{ margin: 0 }}>
									...
									<dd className="v" style={{ margin: 0 }}>
										{announceReading.path.hops}{" "}
										{announceReading.path.hops === 1 ? "hop" : "hops"}
									...
									<dd className="v" style={{ margin: 0 }}>
										{announceReading.appDataLength} B
									</dd>
								</dl>
							</section>
						)}
						...
						{meshcoreAdvert && (
							<section aria-label="MeshCore advertisement">
								...
								<dl className="kv" style={{ margin: 0 }}>
									...
									<dd className="v">
										{meshcoreAdvert.nodeTypeLabel.toUpperCase()}
									</dd>
									{meshcoreAdvert.name && (
										<>
											<dt className="k">NAME</dt>
											<dd className="v" style={{ overflowWrap: "anywhere" }}>
												{meshcoreAdvert.name}
											</dd>
										</>
									)}
									{meshcoreAdvert.hasLocation &&
										meshcoreAdvert.latitude !== undefined &&
										meshcoreAdvert.longitude !== undefined && (
											<>
												<dt className="k">POSITION</dt>
												<dd className="v">
													{meshcoreAdvert.latitude.toFixed(6)}°,{" "}
													{meshcoreAdvert.longitude.toFixed(6)}°
												</dd>
											</>
										)}
						...
						{lxmf && (
							<section aria-label="LXMF message">
								...
								<div className="panel-foot warn">
									Cleartext message · signature not verified
								</div>
								<dl className="kv" style={{ margin: 0 }}>
