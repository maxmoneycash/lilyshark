                                                                 ...
                                                                 std::uint64_t sequence) noexcept
{
    ...
    std::uint8_t header[kLilysharkCaptureRecordHeaderSize]{};
    header[0] = 'L';
    header[1] = 'S';
    header[2] = 'F';
    header[3] = 'R';
    putLe16(header + 4, static_cast<std::uint16_t>(kLilysharkCaptureRecordHeaderSize));
    putLe16(header + 6, 1); // Record layout version.
    putLe16(header + 8, frame.captured_length);
    putLe16(header + 10, frame.original_length);
    putLe64(header + 12, sequence);
    putLe64(header + 20, frame.rf.timestamp_us);
    putLe32(header + 28, frame.rf.present_fields);
    putLe32(header + 32, frame.rf.center_frequency_hz);
    putLe32(header + 36, frame.rf.bandwidth_hz);
    putLe32(header + 40, frame.rf.bit_rate_bps);
    putLe32(header + 44, frame.rf.frequency_deviation_hz);
    putLe32(header + 48, frame.rf.airtime_us);
    putLe32(header + 52, static_cast<std::uint32_t>(frame.rf.frequency_error_hz));
    putLe16(header + 56, static_cast<std::uint16_t>(frame.rf.rssi_dbm_x10));
    putLe16(header + 58, static_cast<std::uint16_t>(frame.rf.snr_db_x10));
    putLe16(header + 60, frame.rf.preamble_symbols);
    putLe16(header + 62, frame.rf.sync_word);
    putLe16(header + 64, frame.rf.profile_id);
    putLe16(header + 66, static_cast<std::uint16_t>(frame.rf.radio_status));
    header[68] = static_cast<std::uint8_t>(frame.rf.tx_power_dbm);
    header[69] = frame.rf.spreading_factor;
    header[70] = frame.rf.coding_rate_denominator;
    header[71] = frame.rf.channel_index;
    header[72] = frame.rf.radio_index;
    header[73] = static_cast<std::uint8_t>(frame.rf.modulation);
    header[74] = static_cast<std::uint8_t>(frame.rf.direction);
    header[75] = static_cast<std::uint8_t>(frame.rf.crc);
    header[76] = metadataFlags(frame.rf);
