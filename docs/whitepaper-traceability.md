...
| Shelby dependency risk: pre-mainnet, no published token terms (§21) | `docs/why-shelby.md` caveats; `analysis/results_evidence.md` (393k blobs, 8.09M txs, dated 2026-08-15) | **evidence** — and the caveat extends to shelbynet's weekly wipe |

The same numbers are quoted on several surfaces (README, docs, INTRO);
each quote is pinned to its source — `analysis/results.md`
...
| Move Prover invariants for score-keeping modules | `field_points` has unit tests and Move Prover formal specifications (`field_points.spec.move`) | **built** |
...
| Capture → Shelby blob → on-chain anchor, each step checkable | Registry deployed to shelbynet; analyzer RESOLVE trace verifies all steps | **partial** — browser publishes anchor via wallet/service; shelbynet anchors are wiped weekly |
| No keys or account on the capture device; a connected host (share service or `shelby-put.ts`) pays for uploads | SHELBY tab step 02; `docs/why-shelby.md` matrix | **built** — replaces the earlier "paid for from the device's own Aptos account" overclaim with the real flow |
...
| Hardware validation: SD writes, touch calibration, scan recovery | README status note | **not built (unverified)** — pending on-device physical validation |
